...
function getActualFacts(facts, schema){
  ...
  facts = facts.filter(fact => fact[factsMapping.added])

  facts.map((fact) => {
    if(fact[factsMapping.attribute] === schema[schemaMapping.endereco][schemaMapping.attribute]){
      let foundAndReplaced = false

     for(let i in actualFacts){
        if(actualFacts[i][factsMapping.entity] == fact[factsMapping.entity] 
          && actualFacts[i][factsMapping.attribute] == fact[factsMapping.attribute]){
            actualFacts[i] = fact
